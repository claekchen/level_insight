  ...
  switch (action.type) {
    case types.PUSH_TO_CONTAINER:
      ...
      if (isExist(action.key, state.companyArray)) {
        return isSingleArray(state.companyArray)
          ? state
          : Object.assign({}, state, {
              companyArray: state.companyArray.filter(
    ...
    case types.DELETE_FROM_CONTAINER:
      if (!isSingleArray(state.companyArray)) {
        return Object.assign({}, state, {
          companyArray: state.companyArray.filter(item => item !== action.key)
        });
      } else {
        return defaultState;
      }
    case types.SELECT_INDUSTRY:
      ...
      defaultState = Object.assign({}, state, {
